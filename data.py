...
class SegmentationAndSCDData(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        self._train = []
        for f in self.protocol.train(): # switch to development set
            file = dict()
        ...
        self._validation = []
        for f in self.protocol.development(): # switch to train set
    ...
    def prepare_chunk(self, file: AudioFile, chunk) -> dict:
        ...
        waveform, _ = self.audio.crop(file, chunk, duration=self.duration)

        # Convert to MFCC if mfcc is set to True
        if self.mfcc:
            sample["X"] = self.mfcc_transform(waveform)
        else:
            sample["X"] = waveform
